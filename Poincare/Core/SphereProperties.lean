...
axiom sphere_path_connected (n : ℕ) (hn : n ≥ 1) :
    PathConnectedSpace (TopCat.sphere n).α

-- S³ 是路径连通的
theorem sphere3_is_path_connected : PathConnectedSpace Sphere3 := by
  unfold Sphere3
  have : (3 : ℕ) ≥ 1 := by norm_num
  exact sphere_path_connected 3 this

-- 路径连通 ⇒ 连通
theorem sphere3_is_connected : ConnectedSpace Sphere3 := by
  haveI := sphere3_is_path_connected
  exact PathConnectedSpace.connectedSpace
...
3. ⏳ `sphere3_is_connected` - S³ 是连通的（通过路径连通性，但路径连通性使用 axiom）
4. ⏳ `sphere3_is_simply_connected` - S³ 是单连通的（需要代数拓扑）
